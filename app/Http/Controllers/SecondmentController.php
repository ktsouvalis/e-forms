...
class SecondmentController extends Controller
{
    ...
    public function update(Secondment $secondment, Request $request){
        if($request->input()['criteriaOrPreferences'] == 1){

        } else {
            //Αν έχει συμπληρώσει επιλογές στον πίνακα προτιμήσεων
        ...
        try{
            ...
            if($request->action == 'submit' && !isset($request->input()['selectionOrder'])){
        ...
        //Αν ζητάει οριστική υποβολή
        //Δημιούργησε το pdf
        try{
            $this->createPDF($secondment, $request->input()['selectionOrder']);
        } catch(\Exception $e) {
            return back()->with('failure', 'Η αίτηση αποθηκεύτηκε αλλά απέτυχε η δημιουργία pdf αρχείου. Προσπαθήστε ξανά. Σε περίπτωση προβλήματος παρακαλούμε για την αποστολή mail στο it@dipe.ach.sch.gr.');
        }
