@if(!$$myapp->stakeholders->count())
    @push('scripts')
        <script src="{{asset('count_criteria_teachers.js')}}"></script>
        <script>
            var countCriteriaTeachersUrl ='{{ route("whocans.count_criteria_teachers", ["access_criteria" =>"mpla"]) }}'
        </script>
    @endpush
    <div class="vstack">
    ...
    @if($$myapp->accessCriteria)
        ...
        @php
            $a_c_id = $$myapp->accessCriteria->id;
        @endphp
        @if($myapp=='microapp')
            <div class="input-group py-1 px-3">
                <button id= "calc_button-{{$a_c_id}}" class="bi bi-arrow-bar-down btn btn-primary criteria-calcbox" data-acid="{{ $a_c_id }}"> Αριθμός Εκπαιδευτικών που ικανοποιούν τα κριτήρια</button>
            </div>
            <div id="calc_acid" class=" py-1 px-3"> </div>
        @endif
    @endif
    </div>
@endif