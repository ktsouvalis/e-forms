<x-layout>
    ...
    @endphp
        @include('microapps.microapps_admin_before') {{-- Visibility and acceptability buttons and messages --}}
            <form action="{{url("/dl_defibrillators_template")}}" method="post">
                @csrf
                <button class="btn btn-secondary bi bi-box-arrow-down"> Πίνακας προς συμπλήρωση </button>
            </form>
            @if(Auth::user()->isAdmin())      
            <nav class="navbar navbar-light bg-light">
                <form action="{{url("/update_defibrillators_template")}}" method="post" enctype="multipart/form-data" class="container-fluid">
                    @csrf
        ...
        @php
            $defibrillators = App\Models\microapps\Defibrillator::all()->sortByDesc('month_id');
        @endphp
            <div class="table-responsive py-2">
                ...
                @foreach($defibrillators as $one)
                    ...
                    <td> {{$one->comments}}</td>
                    <td>
                        <form action="{{url("/dl_defibrillators_file/$one->id")}}" method="post">
                        @csrf
