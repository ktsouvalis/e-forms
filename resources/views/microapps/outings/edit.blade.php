<x-layout_school>
    ...
    <div class="py-3">
            <nav class="navbar navbar-light bg-light">
                    {{-- <form action="{{url("/outings/$outing->id")}}" method="post" enctype="multipart/form-data" class="container-fluid"> --}}
                    <form action="{{route('outings.update', $outing->id)}}" method="post" enctype="multipart/form-data" class="container-fluid">
                        @method('PUT')
                        @csrf
                        ...
                        @else
                            <div class="input-group">
                                ...
                                {{-- <a href="{{url("/outings/$outing->id/edit")}}" class="btn btn-outline-secondary bi bi-arrow-counterclockwise m-2"> Ακύρωση</a> --}}
                                <a href="{{route('outings.edit', $outing->id)}}" class="btn btn-outline-secondary bi bi-arrow-counterclockwise m-2"> Ακύρωση</a>
                            </div>
                        @endif
                    </form>
                </nav>
